...
@HiltViewModel
class TssKeygenViewModel @Inject constructor() : ViewModel() {
...
fun TssKeygenViewModel.createTssKeygenPartyAndWaitPeers(
    keyId: KeyId,
    ...
    println("${keyId.id}:${keyId.key} -> $id")
    val keygenJob = viewModelScope.launch(Dispatchers.IO) {
        println("waiting for everyone")
        val everyone = tssParty.prepareTransport(
            "192.168.1.28:8080",
            id,
            tssKeygenParameters.partyCount.toLong()
        ).split(",").map {
...
fun TssKeygenViewModel.computeKeyShare(step3: TssKeygenPartyStep.TssKeygenStep3): TssKeygenPartyStep.TssKeygenStep4 {
